"""
Core SQLModel models - same tables as before, no data migration needed.
"""
...
class Member(SQLModel, table=True):
    ...
    discord_username: str = Field(max_length=100, unique=True)
    discord_id: Optional[str] = Field(default=None, max_length=50, unique=True)
...
class PromotionLog(SQLModel, table=True):
    ...
    promoted_by: str = Field(max_length=100)
...
class RankMapping(SQLModel, table=True):
    ...
    id: Optional[int] = Field(default=None, primary_key=True)
    system_rank: str = Field(max_length=100, unique=True)
    roblox_role_id: int
